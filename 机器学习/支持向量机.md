...
![vm_0](imgs_md/svm_04.png)
 ...
 因此我们提出一个问题希望能够找到分离样本数据的直线，并且其决策边界越胖越好**（最大边缘超平面，MMH）**。**问题描述**为
$$
...
如下图所示，假设分离平面为$w \cdot x + b =0$，那么可以去适当的值使得上方的支持平面为$w \cdot x + b = 1$，而下方的支持平面为$w \cdot x +b = -1$，具体推导如下：

假设经过支持向量$x_1$的平面为$w_0 x_1 + b_1 = 0$，而下方经支持向量$x_2$的平面为$w_0 x_2 + b_2 = 0$，因为两个平面平行，所以法向量可以是一样的。通过上面式子可以得到$x_1 = -\frac{b_1}{w_0}，x_2=-\frac{b_2}{w_0}$，分别带入二元一次方程组
$$
\begin{cases}
w \cdot x + b = 1 \\
w \cdot x + b = -1
\end{cases}
$$
求解方程组既可以得到w和b满足上述方程组。PS：从中可以看出，如果能够求出支持向量$x_1、x_2$，那么就能够求出分离平面了，但实际上支持向量是需要我们求出来的。

![SVM推导1](imgs_md/SVM推导1.png)

如上图所示，从上面式子可以得到
$$
w \cdot (x_1 - x_2) = 2 \Rightarrow ||w|| \cdot ||x_1 - x_2|| \cdot cos \theta = 2 \\
\Downarrow  \\
||x_1 - x_2|| cos \theta = d 为支撑平面的距离d \\
||w|| \cdot d = 2 \quad \\
\Downarrow  \\
d = \frac{2}{||w||}
$$
对于图中的两个类别，我们需要最大两个支撑平面的距离，设当$w \cdot x_i + b \ge 1$时，其类别为$y_i=1$；而当$w \cdot x_i + b \le -1$时，其类别为$y_i=-1$。联合两种情况可以得到**SVM目标函数**为
$$
max_{w} \frac{2}{||w||} \\
s.t. \quad y_i (w \cdot x_i + b) \ge 1，i=1,2,...,n
$$
将上述问题转换为凸优化问题
$$
min_{w} \frac{||w||^2}{2} \\
s.t. \quad y_i (w \cdot x_i + b) \ge 1，i=1,2,...,n
$$
如下图所示，我们称$H_1$和$H_2$为支撑平面（三维空间），支撑平面是成对出现的。SVM目标就是最大化$H_1$和$H_2$之间的距离，我们要求分离平面在$H_1$和$H_2$之间，距离$H_1$和$H_2$距离相等，称$H$为**最优分离平面（决策边界）**。而$H_1$和$H_2$之间的空间称为**隔离地带**。落在支撑平面上的向量称为**支持向量**，而求支持向量的算法称为**支持向量机**。

![svm_05](imgs_md/svm_05.png)
...
s.t. \quad y_i (w \cdot x_i + b) \ge 1，i=1,2,...,n
$$
上面是凸二次规划问题，什么叫凸？凸集是指有这么一个点的集合，其中任取两个点连一条直线，这条线上的点仍然在这个集合内部，对于凸函数来说，局部最优就是全局最优。对于非约束优化问题采用**朗格朗日乘子法**求解，拉格朗日乘子法的标准格式为
$$
min \ f(x) \\
