...
记L为某模型下样本的似然函数值，k为模型中未知参数的个数(维度)，n为样本数，则有AIC(Akaike Information Criterion)、BIC(Bayesian Information Criterion)[^12]
$$
AIC = -2lnL + 2k \\
BIC = -2lnL + (ln \ n)k
$$
...
[^12]: [BIC](https://en.wikipedia.org/wiki/Bayesian_information_criterion)
