...
如下图所示，黑色是样本点，红色是线性回归曲线，绿色为局部加权回归曲线。图中样本没有明显的线性关系，若采用回归拟合，则效果就不是很好，红色线拟合的情况称为欠拟合(underfit)。如图所示，对x点拟合时，距离x较近的点$x0$对当前$x$的估计影响为$w_0$，同样存在附近的点$x_1,x_2,...$对估计$x$的影响为$w_1,w_2,...$，通过结合权重计算出当前$x$的估计。
