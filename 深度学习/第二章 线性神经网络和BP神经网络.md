...
| 主要人工智能，机器学习，深度学习会议                         | 中文名称                         |
| ------------------------------------------------------------ | -------------------------------- |
| NIPS                                                         | 神经信息处理系统                 |
| ICML                                                         | 国际机器学习大会                 |
| ICLR， International Conference on Learning Representations  |                                  |
| ICASSP                                                       | 国际声学、语音不信号处理会议     |
...
BP神经网络可以用于图像压缩，BP在压缩图像时，一边抽取图像的重要特征，压缩的结果就是图像中最重要的特征的集合，只要保留这些最重要的特征的集合就能够了，在以后可以通过这些重要特征重新添加那些次要特征，这样就能够还原出整个图像，这个抽取特征的过程称之为**自编码器(autoencoder)**。从自编码器出发，研究如何使得自编码器在抽取的过程中自动选择隐层节点的个数(通常远远小于输入节点数)，这个过程称为**稀疏自编码器(sparse autoencoder)**，

依赖BP网络的非线性映射能力进行数据压缩，采用BP神经网络对灰度图像(彩色图像)进行压缩，在保证较好峰值信噪比（PSNR） 的情况下，达到了较高的压缩比。 常见的文件压缩软件如WinZip、 WinRAR等采用的是无损压缩，能够完全恢 复原文件内容。多媒体信息具有信息量大、冗余信息多的特点，往往采用有损压缩技术。

1. 根据大面积着色原理，图像必须在一定面积内存在相同或相似的颜色，对 于人眼的观察来说才有意义，否则看到的只是杂乱无章的雪花。因此，图像中相 邻象素间存在相似性，这样就产生了图像的预测编码。
...
**BP神经网络用于压缩**：只采用一个隐含层，因此整体构成了一个三层的网络。把一组输入模式通过少量的隐含层单元映射到一组输出模式，并使输出模 式尽可能等于输入模式。因此，隐含层神经元的值和相应的权值向量可以输出 一个与原输入模式相同的向量。当隐含层的神经元个数较少时，就意味着**隐含层能用更少的数来表现输入模式**，而这，实际上就是**压缩** 。

第一层为输入层，中间层为隐含层，网络的映射功能依赖隐含层实现。 输入层到隐含层的变换相当于压缩的编码过程；而从隐含层到输出层的变换则相当于解码过程。

![自编码器](md_img/自编码器.png)

输入层和输出层均由M个神经元组成 ，隐含层包含K个神经元 。网络 对数据进行压缩编码，保存在隐含层，再由隐含层输出到下一层。

![自编码器2](md_img/自编码器2.png)

假设输入图像为像素$N*N$大小，被细分为多个$n*n$的图像块。如果将图像块中每一个像素点与一个输入或输出神经元相对应，则有

![自编码器3](md_img/自编码器3.png)

网络随机地抽取图像中各n*n图像块作为学习模式，使用反向传播算法进行学习，通过调整网络中神经元之间的连接权值，使训练集图像的重建误差$E=f-g$的均值达到最小。 
...
**图像块划分举例**：对于$128 \times 128$的图像，以$4 \times 4$为块划分，每一块作为一个输入，将$4 \times 4$的块拉伸为$16 \times 1$的列向量，则构成$16 \times 1024$矩阵，每一列为一个输入样本，总共有1024个样本。

![自编码器4-图像块划分](md_img/自编码器4-图像块划分.png)

**归一化**：神经网络的输入样本一般都需要进行归一化处理，这样更能保证性能的稳定性。归一化可以使用mapminmax函数进行，考虑到图像数据的特殊性，像素点灰度值为整数，且处于0-255之间，因此归一化处理统一 将数据除以255即可。    

**BP网络训练**：BP网络的输入有16个$x_1 \sim x_{16}$，即变量个数为16，而中间隐藏层的个数通常少于16，输出值$y_1 \sim y_{16}$个数也为16。将输出值$y_1 \sim y_{16}$与输入值$x_1 \sim x_{16}$之间的误差平方和函数MSE作为目标函数或损失函数。压缩过程体现在：因为中间隐藏层节点数少于输入节点数，从而实现数据的降维，中间隐藏层的数据大小就肯定比输入数据小。**这个降维过程可以看做是去噪过程，特征提取过程**。

**稀疏编码器**

稀疏自动编码器的目的是让其能够自动决定中间隐藏层节点数，稀疏即使得隐藏层中大部分权值为0而只有少数几个节点的权值有效。如下图所示，目标函数为$error = |prediction - input|^2$，训练时采用后向传播算法，**若想要中间层变成稀疏**，则只需要在目标函数中加上一个**稀疏惩罚项**，所谓稀疏惩罚项即对隐藏层的输出值求其$L1或L2$正则项(绝对值之和、绝对值平方之和)，这个目标函数不仅仅是预测值与输入值的差，还需要考虑中间隐藏层数值的大小，若中间层有很多非零则目标函数会很大，所以计算出来的结果会自动均衡，即得到的结果的图像的差别不是很大，并且中间层要有很多零(节点数越少)越好，这就是**稀疏编码器的**由来。

![自编码器5](md_img/自编码器5.png)

具体参见[^3]有关稀疏自编码器的阐述。

**稀疏自动编码器的意义**：每一个隐藏层节点如果能够稀疏表达的话，它其实是提取出该图中变化比较大的特征，这里是指边缘特征，例如一张桌子的边或角，人的五官的轮廓等变化比较大的地方[^3]。
...
[^3]: Andrew Ng CS294A Lecture notes：Sparse autoencoder
